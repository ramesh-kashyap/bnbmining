...
class BonusController extends Controller
{
    public function roi_bonus(Request $request)
    {
           ...
           $limit = $request->limit ? $request->limit :  paginationLimit();
            ...
            $notes = Income::where('remarks','Global Income')->orderBy('id', 'DESC');
    ...
    public function level_bonus(Request $request)
    {
           ...
           $limit = $request->limit ? $request->limit :  paginationLimit();
            $status = $request->status ? $request->status : null;
            $search = $request->search ? $request->search : null;
            $notes = Income::where('remarks','Team Build Income')->orderBy('id', 'DESC');
           if($search <> null && $request->reset!="Reset"){
            $notes = $notes->where(function($q) use($search){
    ...
    public function global_team_bonus(Request $request)
    {
           ...
           $limit = $request->limit ? $request->limit :  paginationLimit();
            $status = $request->status ? $request->status : null;
            $search = $request->search ? $request->search : null;
            $notes = Income::where('remarks','Global Team Boost Income')->orderBy('id', 'DESC');
           if($search <> null && $request->reset!="Reset"){
            $notes = $notes->where(function($q) use($search){
              $q->Where('rname', 'LIKE', '%' . $search . '%')
              ->orWhere('ttime', 'LIKE', '%' . $search . '%')
            ...
            $notes = $notes->paginate($limit)
                ...
                $this->data['level_incomes'] =  $notes;
                $this->data['search'] = $search;
                $this->data['page'] = 'admin.bonus.global_team_bonus';
                return $this->admin_dashboard();
    }
