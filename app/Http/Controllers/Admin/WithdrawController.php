...
class WithdrawController extends Controller
{
    ...
    public function pendingWithdrawal(Request $request)
    {
      ...
      public function payWithdrawal(Request $request)
    {
        $limit = $request->limit ? $request->limit : 100000000000;
        $status = $request->status ? $request->status : null;
        $search = $request->search ? $request->search : null;
        ...
        $this->data['withdraw_list'] =  $notes;
        $this->data['search'] = $search;
        $this->data['page'] = 'admin.withdraw.pay-withdraw';
        return $this->admin_dashboard();
    }
    ...
    public function approvedWithdrawal(Request $request)
    {
        ...
        {
        $net_amt= $withdraw_id->amount-$withdraw_id->amount*50/100;
        $resultAarray = Helper::CreateWithdrawal($net_amt,'BUSD.BEP20',$withdraw_id->account,1);   
        
        // dd($resultAarray);
         if($resultAarray['error']=="ok")
        {
         $update_data['txn_id']= $resultAarray['result']['id'];     
        }
        else
        {
         $encodedSku = json_encode($resultAarray);   
          $update_data['response']= $encodedSku; 
        }
            
        $update_data['status']= 'Approved'; 
        $update_data['paid_date']= Carbon::now(); 
        add_level_boots($user_id->id,$withdraw_id->amount*25/100);
