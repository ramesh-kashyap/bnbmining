    ...
    public function SubmitTicket(Request $request)
    {
      ...
      try{
            ...
            if($validation->fails()) {
             ...
             $user=Auth::user();
            $invest_check=Ticket::where('user_id',$user->id)->where('status',0)->orderBy('id','desc')->limit(1)->first();
            if($invest_check)
            {
                 return Redirect::back()->withErrors(array('already your ticket is pending!!'));
            }
            ...
            $message=$request->message;
                ...
                $ticket_no_final = substr(time(),-4).substr(rand(),-4).substr(mt_rand(),-3);
