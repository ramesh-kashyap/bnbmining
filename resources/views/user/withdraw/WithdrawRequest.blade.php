                    ...
                    <div class="withdraw">
                        ...
                        <form onsubmit="return amtValue()" method="post" action="{{ route('user.Withdraw-Request') }}">
                            {{ csrf_field() }}
                                    ...
                                    <input type="text" name="balance"  value="{{ number_format(Auth::user()->available_balance(), 2) }}" disabled="disabled"  class="def-input" />

                                    <input type="text"style="margin-top:15px;" name="trx_address"  value="{{Auth::user()->walletAddress}}" disabled="disabled"  class="def-input" />
                                    <input type="text" style="margin-top:15px;"name="amount" id="PACKAGE_AMT" required  class="def-input" placeholder="Withdrawal Amount" />
                            ...
                            <!-- Submit Button -->
                            <button type="submit" style="margin-top:25px;" class="btn btn-primary def-btn">Withdraw</button>
                        </form>
                    </div>
...
@foreach($withdraw_report as $value)
                                <tr>
                                    ...
                                    <td  class="empty">{{ ($value->amount) }} <span style="font-size: 12px">{{currency()}}</span></td>
